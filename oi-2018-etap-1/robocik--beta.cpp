...
int main() {
  ...
  // cmp(f(2, 0, 13, 0, 0, 0, 3, 0), 4);
  // cmp(f(2 + 10, 0, 13 + 10, 0, 0 + 10, 0, 3, 0), 4);
  // cmp(f(2, 30, 13, 30, 0, 30, 3, 0), 4);

  // cmp(f(-100, -1, 0, -1, 0, -1, 3, 0), 1);
  // cmp(f(-100, -1, 3, -1, 0, -1, 3, 0), 2);
  // cmp(f(-100, -1, -1, -1, 0, -1, 3, 0), 0);

  // cmp(f(-100, -1, 3 * 100, -1, 0, -1, 3, 0), 101);

  // cmp(f(0, -1, 100, -1, 0, -1, -3, 0), 1);
  // cmp(f(0, -1, 100, -1, 2, -1, -3, 0), 1);
  // cmp(f(0, -1, 100, -1, 2, 1, -3, 0), 0);

  // cmp(f(2, -1, 2, -5, 5, 2, -1, -2), 1);
  // cmp(f(2, -1, 2, -5, 5, 1, -1, -2), 1);
  // cmp(f(2, -1, 2, -5, 5, 0, -1, -2), 0);
  // cmp(f(2, -1, 2, -5, 2, 2, -1, -2), 0);
  // cmp(f(2, -1, 2, -5, 1, 2, -1, -2), 0);

  // cmp(f(3, -1, 10, -1, 2, -1, 2, 0, 0), 0);
  // cmp(f(3, -1, 10, -1, 2, -1, 2, 0, 1), 1);
  // cmp(f(3, -1, 10, -1, 2, -1, 2, 0, 2), 2);
  // cmp(f(0, 10, 0, 1, 2, 16, -2, -8, 2), 1);
  // cout << endl;
